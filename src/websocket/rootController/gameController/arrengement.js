...
async function arrengement_getUsingGroup (msg, socket) {
  const userInfo = await arrengementLib.findCardGroup({
    username: socket.glory.userInfo.username
  })
  ...
  socket.$emit('game', {
    ...
    content: userInfo.arrengement.usingGroup
  })
}

async function arrengement_updateUsingGroup (msg, socket) {
  await arrengementLib.updateUsingGroup({username: socket.glory.userInfo.username}, msg)

  arrengement_getUsingGroup(null, socket)
}
